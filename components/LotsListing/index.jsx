...
const LotsListing = ({ lotsData, arrowIcon, homeFilterIcon, defaultImage }) => {
  ...
  const renderlotsData = useMemo(() => {
    ...
    if (lotsDataFiltered.length === 0 && !isGroupView) {
      return (
        <div className={style.noLotsFound}>
          Nenhum imóvel encontrado, entre em contato para mais informações.
        </div>
      );
    }
      ...
      <Link href={`/lote/${item.id}`} key={item.id}>
        <li>
          <div className={style.lotImage}>
            <img
              src={item.loteImages[0]?.imageUrl || defaultImage}
              alt="Lote"
            />
          </div>
          <div className={style.lotInfo}>
            ...
            <div className={style.lotItemActions}>
              <span className={style.itemPrice}>
                {item.hiddenPrice ? (
                  <span className={style.hiddenPrice}>Preço sob consulta</span>
                ) : (
      ...
      <div className={style.heading}>
        ...
        <div className={style.filterBtns}>
          <button
            ...
            className={`${style.groupByLoteamento} ${
              isGroupView && style.active
            }`}
          >
            Agrupar por loteamento
          </button>
          <button className={style.filterBtn} onClick={handleShowFilters}>
            <img src={homeFilterIcon} alt="Filtrar" />
            Filtros
          </button>
        </div>
        {showListFilters && (
          ...
          {totalPages > 0 && (
            <div className={style.lotsListingControls}>
              {currentPage > 1 && (
                ...
                >
                  Anterior <img src={arrowIcon} alt="Anterior" />
                </button>
              )}
              <button onClick={goToFirstPage} className={style.currentPage}>
                {currentPage}{" "}
              </button>
              <span>de</span>
              <button onClick={goToLastPage} className={style.totalPages}>
                {" "}
                {totalPages}
              </button>
