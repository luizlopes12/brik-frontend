      ...
      case "left":
        ...
        if (window.innerWidth < 768) {
          carouselRef.current.scrollLeft =
            carouselRef.current.scrollLeft -
            (carouselRef.current.querySelector("div").clientWidth + 18);
        } else {
          carouselRef.current.scrollLeft =
            carouselRef.current.scrollLeft -
            (carouselRef.current.clientWidth - 120);
        }
        break;
      case "right":
        currentPage < Math.ceil(lotsDataFiltered.length / lotsDataPerPage) &&
            ...
            <div className={style.lotItem} key={item.id}>
              <Link href={`/lote/${item.id}`}>
                <div className={style.lotImage}>
                  <img
                    src={
                      item.loteImages[0]?.imageUrl ||
                      defaultImage ||
                      "https://i.imgur.com/tKxr7jg.png"
                    }
                    alt="Lote"
                  />
                </div>
                <div className={style.lotInfo}>
                  <div className={style.lotInfos}>
                    <span className={style.itemPrice}>
                      {item.hiddenPrice ? (
                        <span className={style.hiddenPrice}>
                          Preço sob consulta
                        </span>
                      ) : (
