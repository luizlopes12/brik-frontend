...
const LotRegisterPopUp = () => {
  ...
  const handleDeletePartnerFromLot = (partnerToDelete) => {
    setLotData((prev) => ({
      ...prev,
      partners: prev.partners.filter((partner) => partner !== partnerToDelete),
    }));
    setTimeout(() => {
      setEditPartnerFromLot(null);
    }, 0);
        ...
        await fetch(`${process.env.BACKEND_URL}/lots/add`, {
          ...
          .then(async (data) => {
            ...
            lotImages.forEach(async (image) => {
              ...
              await fetch(
                `${process.env.BACKEND_URL}/lots/${data.newLot?.id}/images/add`,
                {
                  method: "POST",
                  headers: {
                    "Content-Type": "application/json",
            ...
            lotData.partners.forEach(async (partner) => {
              ...
              await fetch(
                `${process.env.BACKEND_URL}/lots/${partner.idLote}/partners/add`,
                {
                  method: "POST",
                  headers: {
                    "Content-Type": "application/json",
  ...
  const getTaxes = async () => {
    await fetch(`${process.env.BACKEND_URL}/taxes/list`)
      ...
      .then((data) => {
        ...
        setLotData((prev) => ({
          ...prev,
          taxPercentage: parseFloat(data[0].defaultTax),
          taxPercentage24: parseFloat(data[0].after24Months),
        }));
      });
  };
  useEffect(() => {
  ...
  return (
    <div
      className={popUps.lotRegister ? style.popUpBackdrop : style.popUpDisabled}
    >
                ...
                {!lotData.hiddenPrice && (
                  <div className={style.priceInputs}>
                    ...
                    <input
                      ...
                      min="0"
                      name="price"
                      value={formatPrice(lotData.price)}
                      onChange={(event) => {
                        handleLotData({
                          target: {
                ...
                <div className={style.typeOptions}>
                  <span className={style.typeInput}>
                    ...
                    <input
                      ...
                      value="Rural"
                      name="lotType"
                      onClick={() => handleSetLotType("rural")}
                      checked={lotType.rural}
                  ...
                  <span className={style.typeInput}>
                    ...
                    <input
                      ...
                      value="Rural"
                      name="lotType"
                      onClick={() => handleSetLotType("urbano")}
                      checked={lotType.urbano}
                      className={style.seletionRadio}
                    />
                  </span>
                </div>
