...
const LotRegisterPopUp = () => {
  ...
  useEffect(() => {
    let selectedLotDivisionData = globalDivisionsData.find(
      (division) => division.id === lotSelected.idLoteamento
    );
    setLotDivision({
  ...
  const handleExitPopUp = () => {
    ...
    setLotImages(["/images/labels/without-image.png"]);
    setLotDivision({
      name: "Selecione um Loteamento",
      logoUrl: "https://i.imgur.com/YQOzMWA.png",
      id: 0,
  ...
  const handleUploadImage = (e) => {
    ...
    if (!image) {
      ...
      const storageRef = ref(
        storage,
        `files/lotes/${currentDate}_${image.name}`
      );
      ...
      uploadTask.on(
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
            setLotImages((prev) => {
              imagesCarouselRef.current.scrollLeft +=
                imagesCarouselRef.current.offsetWidth;
              return [...prev, downloadURL];
            });
            setLotImages((prev) =>
              prev.filter(
                (element) => element !== "/images/labels/without-image.png"
              )
            );
          });
        }
  ...
  const handleAddParcel = () => {
    setLotData((prev) => ({
      ...prev,
      maxPortionsQuantity: prev.maxPortionsQuantity + 1,
    }));
  };
  const handleRemoveParcel = () => {
    setLotData((prev) => ({
      ...prev,
      maxPortionsQuantity:
        prev.maxPortionsQuantity > 0 && prev.maxPortionsQuantity - 1,
    }));
  };
  ...
  const handleDeletePartnerFromLot = (partnerToDelete) => {
    setLotData((prev) => ({
      ...prev,
      lotePartners: prev.lotePartners.filter(
        (partner) => partner !== partnerToDelete
      ),
    }));
  ...
  const handleAddPartner = (partnerData) => {
    if (!lotData.lotePartners.includes(partnerData)) {
      setLotData((prev) => ({
        ...prev,
        lotePartners: [...prev.lotePartners, partnerData],
      }));
  ...
  const handleCreateNewPartner = () => {
    setShowNewPartnerInputs((prev) => {
      if (
        prev & (newPartner.name.length > 0) &&
        newPartner.CPF.length > 0 &&
        newPartner.percentage.length > 0
      ) {
        setLotData((prev) => ({
          ...prev,
          lotePartners: [...prev.partners, newPartner],
        }));
  ...
  const lotDivisionFiltered = useMemo(() => {
    return lotDivision.divisionPartners
      ?.filter((partner) =>
        partner.name?.toLowerCase().includes(partnerSearchInput.toLowerCase())
      )
      .filter((partnerToNotDisplay) =>
        lotSelected.lotePartners.every(
          (partner) => partnerToNotDisplay.CPF !== partner.CPF
        )
      );
  }, [
    lotDivision.divisionPartners,
    showDivisionOptions,
    partnerSearchInput,
    lotSelected.lotePartners,
  ]);
      ...
      await fetch(`${process.env.BACKEND_URL}/lots/edit/${lotData.id}`, {
        ...
        .then(async (data) => {
          ...
          lotImages.forEach(async (image) => {
            ...
            await fetch(
              `${process.env.BACKEND_URL}/lots/${lotData.id}/images/add`,
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
          ...
          if (deletedPartners.length > 0) {
            deletedPartners.forEach(async (partner) => {
              ...
              await fetch(
                `${process.env.BACKEND_URL}/lots/partners/delete/${lotData.id}/${partner.id}`,
                {
                  method: "DELETE",
                  headers: {
                    "Content-Type": "application/json",
          ...
          if (newPartnersAdded.length > 0) {
            newPartnersAdded.forEach(async (partner) => {
              ...
              await fetch(
                `${process.env.BACKEND_URL}/lots/${lotData.id}/partners/add`,
                {
                  method: "POST",
                  headers: {
                    "Content-Type": "application/json",
          ...
          if (partnersUpdated.length > 0) {
            partnersUpdated.forEach(async (partner) => {
              ...
              await fetch(
                `${process.env.BACKEND_URL}/lots/partners/edit/${partner.id}`,
                {
                  method: "PATCH",
                  headers: {
                    "Content-Type": "application/json",
              ...
              <div className={style.lotImagesCarrousel} ref={imagesCarouselRef}>
                ...
                {lotImages.length > 1 && (
                  <ul className={style.carrouselControllers}>
                    <li
                      key={0}
                      className={style.controlPrev}
                      onClick={() => imageSlide("prev")}
                    >
                      <img src="/images/prevIcon.svg" />
                    </li>
                    <li
                      key={1}
                      className={style.controlNext}
                      onClick={() => imageSlide("next")}
                    >
                      <img src="/images/nextIcon.svg" />
                    </li>
                  </ul>
                )}
              </div>
          ...
          <div className={style.lotDataInputs}>
            <div className={style.locationArea}>
              ...
              <div className={style.locationInputs}>
                <input
                  type="text"
                  placeholder="Av. Exemplo, Vila Romão, Registro - SP"
                  name="location"
                ...
                {!lotData.hiddenPrice && (
                  <div className={style.priceInputs}>
                    ...
                    <input
                      ...
                      min="0"
                      name="finalPrice"
                      value={formatPrice(lotData.finalPrice)}
                      onChange={(event) => {
                        handleLotData({
                          target: {
                ...
                {!lotData.hiddenPrice && (
                  <div className={style.basePriceInput}>
                    ...
                    <input
                      ...
                      min="0"
                      name="basePrice"
                      value={formatPrice(lotData.basePrice)}
                      onChange={(event) => {
                        handleLotData({
                          target: {
                            name: event.target.name,
              ...
              <div className={style.parcelQuantity}>
                ...
                <div className={style.parcelQuantityInput}>
                  ...
                  <input
                    ...
                    min="0"
                    name="finalPrice"
                    value={lotData.maxPortionsQuantity}
                    onChange={handleLotData}
                  />
              ...
              <div className={style.taxes}>
                ...
                <div className={style.basePriceInput}>
                  <input
                    ...
                    max="100"
                    min="0"
                    name="taxPercentage24"
                    value={lotData.taxPercentage24}
                    onChange={handleLotData}
                  />
                  <span>%</span>
                </div>
            ...
            <div className={style.lotStatus}>
              <div>
                ...
                <p className={style.statusInfo}>
                  Esse lote está disponível para venda?{" "}
                </p>
              </div>
              <div className={style.lotStatusSelect}>
                <select
                  onChange={handleSelectFilters}
                  value={selectValues.isAvaible}
                  name="isAvaible"
          ...
          <div className={style.lotDataActions}>
            <div className={style.partnersArea}>
              <div className={style.lotDataActionsHeader}>
                ...
                {editPartnerFromLot != null && (
                  <button
                    className={style.deletePartner}
                    onClick={(e) =>
                      handleDeletePartnerFromLot(editPartnerFromLot)
                    }
                  >
                  ...
                  <div className={style.partnersListItemWrapper}>
                    {lotData.lotePartners.map((partner, index) => (
                      <li
                        key={index}
                        className={
                          partner == editPartnerFromLot && !showNewPartnerInputs
                            ? style.partnersListItemEdit
                            : style.partnersListItem
                        }
                        onClick={() => handlePartnerActions(partner)}
                      >
                        <input
                          value={partner.name}
              ...
              <div className={style.addPartner}>
                {showNewPartnerInputs ? (
                  <button
                    ...
                    className={style.addNewPartnerToLot}
                  >
                ...
                <button
                  onClick={() => handleShowPartnersOptions()}
                  className={style.addPartnerToLot}
                >
                  {showPartnersOptions ? (
                    <>
                    ...
                    <>
                      <img src="/images/partnersIcon.svg" />
                      Adicionar sócio
                    </>
                  )}
                </button>
                  ...
                  <ul className={style.partnersOptionsSelectorList}>
                    {lotDivisionFiltered?.length > 0 ? (
                      lotDivisionFiltered.map((partner, index) => (
                        <li
                          key={index}
                          className={style.lotDivisionPartnersItem}
            ...
            <div className={style.saveLotData}>
              <button
                ...
                className={style.saveLotDataButton}
              >
