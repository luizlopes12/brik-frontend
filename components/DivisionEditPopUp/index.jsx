...
const DivisionEditPopUp = () => {
  ...
  const handleUploadLogo = async (e) => {
    ...
    if (!image) {
      setAlertMessage((prev) => ({
        ...prev,
        logo: "Não foi possível realizar o upload, tente novamente.",
      }));
      ...
      const storageRef = ref(
        storage,
        `files/logos/${currentDate}_${image.name}`
      );
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
            ...
            setSuccessMessage((prev) => ({
              ...prev,
              logo: "Upload realizado com sucesso.",
            }));
            setTimeout(
              () => setSuccessMessage((prev) => ({ ...prev, logo: "" })),
              5000
  ...
  const handleUploadBlueprint = (e) => {
    ...
    if (!image) {
      setAlertMessage((prev) => ({
        ...prev,
        bluePrint: "Não foi possível realizar o upload, tente novamente.",
      }));
      ...
      const storageRef = ref(
        storage,
        `files/bluePrints/${currentDate}_${image.name}`
      );
      const uploadTask = uploadBytesResumable(storageRef, image);
      uploadTask.on(
        "state_changed",
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
            ...
            setSuccessMessage((prev) => ({
              ...prev,
              bluePrint: "Upload realizado com sucesso.",
            }));
            setTimeout(
              () => setSuccessMessage((prev) => ({ ...prev, bluePrint: "" })),
              5000
  ...
  const handleSaveData = async () => {
    ...
    await fetch(
      `${process.env.BACKEND_URL}/divisions/edit/${divisionData.id}`,
      {
        method: "PATCH",
        headers: {
      ...
      .catch((err) => {
        ...
        setTimeout(
          () => setAlertMessage((prev) => ({ ...prev, save: "" })),
          5000
        );
      });
  };
  ...
  const handleAddPartnertoDivision = async () => {
    if (
      partnerValues.name?.length > 0 &&
      partnerValues.CPF?.length >= 11 &&
      partnerValues?.percentage > 0
    ) {
      setPartnerPopUp(false);
      await fetch(
        `${process.env.BACKEND_URL}/divisions/${divisionData.id}/partners/add`,
        {
          method: "POST",
          headers: {
            "Content-type": "application/json",
        ...
        .then((data) => {
          setDivisionSelected((prev) => ({
            ...prev,
            divisionPartners: data.partnersList,
          }));
        });
      setPartnerData({});
    } else {
      setAlertMessage((prev) => ({
        ...prev,
        partner: "Preencha todos os campos corretamente.",
      }));
      setSuccessMessage((prev) => ({ ...prev, partner: "" }));
      setTimeout(
        () => setAlertMessage((prev) => ({ ...prev, partner: "" })),
        5000
      );
    }
  };
  ...
  return (
    <div
      className={
        popUps.divisionEdit ? style.popUpBackdrop : style.popUpDisabled
      }
    >
      <div className={style.popUpWrapper}>
          ...
          <ul className={style.popUpsInputs}>
            <li className={style.inputField}>
              <input
                ...
                name="name"
                onChange={(e) => handleDivisionData(e)}
              />
            </li>
            <li className={style.inputField}>
              <span>
              ...
              <input
                value={divisionData.location}
                name="location"
                onChange={(e) => handleDivisionData(e)}
              />
            </li>
            {alertMessage.logo.length > 0 && (
              <p className={style.alertMessage}> {alertMessage.logo} </p>
            )}
            {successMessage.logo.length > 0 && (
              <p className={style.successMessage}> {successMessage.logo} </p>
            )}
          </ul>
        </div>
            ...
            {partnerPopUp && (
              <li className={style.partnerAddForm}>
                <input
                  ...
                  onChange={handlePartnerData}
                />
                <input
                  type="text"
                  placeholder="CPF"
                  name="CPF"
                  value={partnerValues.CPF}
                  onChange={handlePartnerData}
                />
                <input
                  type="number"
                  placeholder="%"
                  name="percentage"
                  value={partnerValues.percentage}
                  max="100"
                  min="0"
                  onChange={handlePartnerData}
                />
          ...
          {partnerPopUp ? (
            <div className={style.confirmPartnerWrapper}>
              <span className={style.alertMessage}>
                {" "}
                {alertMessage.partner.length > 0 && alertMessage.partner}{" "}
              </span>
              <button
                className={style.addPartnerBtn}
                onClick={handleAddPartnertoDivision}
                name="addPartner"
              >
                <img src="/images/confirm.svg" /> Confirmar
              </button>
            </div>
          ) : (
            <button
              ...
              name="addPartner"
            >
              <img src="/images/plusIcon-green.svg" /> Adicionar sócio
            </button>
          )}
        </div>
        <div className={style.bluePrint}>
          <a
            className={style.downloadBlueprint}
            href={divisionData.bluePrint}
            target="_blank"
          ...
          <form ref={uploadBlueprintForm}>
            <a className={style.uploadBlueprint}>
        ...
        {alertMessage.bluePrint.length > 0 && (
          <p className={style.alertMessage}> {alertMessage.bluePrint} </p>
        )}
        {successMessage.bluePrint.length > 0 && (
          <p className={style.successMessage}> {successMessage.bluePrint} </p>
        )}
        <button className={style.saveBtn} onClick={handleSaveData}>
