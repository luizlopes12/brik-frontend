...
export async function getStaticPaths() {
    const res = await fetch(`${process.env.BACKEND_URL_LOCAL}/lots/list`);
...
export async function getStaticProps({ params }) {
    const res = await fetch(`${process.env.BACKEND_URL_LOCAL}/lots/${params.id}`);
    const data = await res.json();
    let data2 = await fetch(`${process.env.BACKEND_URL_LOCAL}/divisions/list`)
...
const Form = ({ lotData, divisionData }) => {
  ...
  return (
    <section className={style.formContainer}>
      <Typography variant="h4" component="h1" style={{textAlign: 'center', fontWeight: 'bold', color: '#292929'}}>
      ...
      />
            <TextField
        name="email"color="success"
        label="Email"
        value={formData.email}
        onChange={handleInputChange}
      />
      <TextField
        name="cpf"color="success"
        label="CPF"
        value={formData.cpf}
        onChange={handleInputChange}
      />
      <TextField
        name="rg"color="success"
        label="RG"
        value={formData.rg}
        onChange={handleInputChange}
      />
      <TextField
        name="endereco"color="success"
        label="Endereço"
        value={formData.endereco}
        onChange={handleInputChange}
      />
      <TextField
        name="nacionalidade"color="success"
        label="Nacionalidade"
        value={formData.nacionalidade}
        onChange={handleInputChange}
      />
      <TextField
        name="profissao"color="success"
        label="Profissão"
        value={formData.profissao}
        onChange={handleInputChange}
      />
      <TextField
        name="telefone"color="success"
        label="Telefone"
